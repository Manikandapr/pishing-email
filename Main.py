...
ttsplit(x_new,y_new,test_size=0.2,shuffle=True)
...
pickle.dump({'classifier':classifier,'message_x':message_x},open("training_d
...
Or Ham ? Message Detector",

font=FONT_BOLD%(18),pady=10)

mid_label.place(relwidth=1,rely=0.12)

 #answer label where our prediction about user input message will be 
...
self.msg_entry=Text(self.window,bg="#FFF",fg="#000",font=FONT_BOLD
...
("spam" if self.classifier.predict(self.bow(message)).reshape(1,-1) else 
